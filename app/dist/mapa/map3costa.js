...
// Capas	  
	var costa_normativa_norte = new ol.layer.Vector({
		style: styleFunction,
        source: new ol.source.Vector({
        url: function(extent) {
          return 'http://geoserver.fadu.edu.uy:8080/geoserver/apps/wfs?service=WFS&' +
              'version=1.0.0&request=GetFeature&typeName=apps:costaplan_padrones_norte&' +
		...
		var costa_terreno_norte = new ol.layer.Vector({
        source: new ol.source.Vector({
		style: styleFunction,
        url: function(extent) {
          return 'http://geoserver.fadu.edu.uy:8080/geoserver/apps/wfs?service=WFS&' +
...
//Mapa
	var map = new ol.Map({
		target: 'map',  
		renderer: 'canvas', 
		layers: [BaseNeutra, costa_normativa_norte, costa_terreno_norte],     
		view: new ol.View({
			projection: projection,
			center: [595000,6148250],
			zoom: 16,
			maxZoom: 18,
			minZoom: 14,
...
//Funciones de los switch
	var normativa = document.getElementById("normativa");
        ...
        if (normativa.checked == true) {
           	costa_normativa_norte.setVisible(true);
			costa_normativa_norte.setZIndex (2);
		   
        } else {
            costa_normativa_norte.setVisible(false);
        }
    }
	...
	var terreno = document.getElementById("terreno");
        ...
        if (terreno.checked == true) {
           costa_terreno_norte.setVisible(true);
		   
        } else {
            costa_terreno_norte.setVisible(false);
        }
    }
